...
public class PackAsset : IDispose
{
    ...
    //泛型获取 异步
    public void getObj<T>(string name, Action<T,string> callBack) where T : UnityEngine.Object
    {
        LoadThread.Instance.StartCoroutine(getObjAsync(name, callBack));
    }

    IEnumerator getObjAsync<T>(string name, Action<T,string> callBack) where T : UnityEngine.Object
    {
        AssetBundleRequest req = ab.LoadAssetAsync<T>(name);
        yield return req;
        callBack(req.asset as T,resName);
    }
